...
public static class DatabaseSeeder
{
    public static async Task SeedAsync(EvidenceDbContext db)
    {
        ...
        if (db.Database.IsSqlite())
            await UpgradeExistingSqliteAsync(db);
        ...
        if (!await db.Users.AnyAsync(x => x.Username == "investigator"))
        {
            var investigator = new AppUser
            {
            ...
            investigator.PasswordHash = hasher.HashPassword(
                investigator,
                "Investigator@123!"
            );

            db.Users.Add(investigator);
        }

        // Evidence Custodian
        if (!await db.Users.AnyAsync(x => x.Username == "evidence"))
        {
            var evidence = new AppUser
            {
                Username = "evidence",
                FullName = "Hrithik",
            ...
            evidence.PasswordHash = hasher.HashPassword(
                evidence,
                "Evidence@123!"
            );

            db.Users.Add(evidence);
        }


        if (!await db.Users.AnyAsync(x => x.Username == "auditor"))
        {
            ...
            auditor.PasswordHash = hasher.HashPassword(
                auditor,
                "Auditor@123!"
            );

            db.Users.Add(auditor);
        }


        if (!await db.Users.AnyAsync(x => x.Username == "forensic"))
        {
            var forensic = new AppUser
            {
                Username = "forensic",
            ...
            forensic.PasswordHash = hasher.HashPassword(
                forensic,
                "Forensic@123!"
            );

            db.Users.Add(forensic);
        }
        ...
        if (!await db.Cases.AnyAsync())
        {
            var cases = new[]
            {
                new CaseFile
                {
                    ...
                    CaseType = "Cybercrime",
                    Description = "Financial cyber-fraud investigation.",
                    Status = "In Progress",
                    Priority = "High",
                    Investigator = "Wasi Ahbab",
                    OpenedOn = DateTime.Today.AddDays(-12)
                },
                ...
                new CaseFile
                {
                    ...
                    CaseType = "Device Examination",
                    Status = "Open",
                    Priority = "Medium",
                    Investigator = "Wasi Ahbab",
                    OpenedOn = DateTime.Today.AddDays(-7)
                },

                new CaseFile
                {
                    CaseNumber = "CF-2026-003",
                    Title = "Data Breach Review",
                    CaseType = "Incident Response",
                    Status = "Pending Review",
                    Priority = "High",
                    Investigator = "Wasi Ahbab",
                    OpenedOn = DateTime.Today.AddDays(-3)
                }
            };
            ...
            db.Evidence.AddRange(
                new EvidenceItem
                {
                    ...
                    Name = "Laptop disk image",
                    Type = "Hard drive",
                    Status = "In Examination",
                    StorageLocation = "Locker A-04",
                    CurrentCustodian = "Wasi Ahbab",
                    Collector = "Intake Unit",
                    CaseFileId = cases[0].Id,
                    CollectedOn = DateTime.Today.AddDays(-11)
                },
                ...
                new EvidenceItem
                {
                    ...
                    Type = "Mobile phone",
                    Status = "Received",
                    StorageLocation = "Locker B-02",
                    CurrentCustodian = "Evidence Custodian",
                    Collector = "Intake Unit",
                    CaseFileId = cases[1].Id,
                    CollectedOn = DateTime.Today.AddDays(-6)
                },

                new EvidenceItem
                {
                    EvidenceNumber = "EV-003",
                    Name = "Network log archive",
                    Type = "Network capture",
                    Status = "Reviewed",
                    StorageLocation = "Digital Vault",
                    CurrentCustodian = "Forensic Analyst",
                    Collector = "SOC Team",
                    CaseFileId = cases[2].Id,
                    CollectedOn = DateTime.Today.AddDays(-2)
                }
            );
            ...
            db.ForensicAnalyses.Add(
                new ForensicAnalysis
                {
                    EvidenceItemId = 1,
                    Analyst = "Wasi Ahbab",
                    Status = "In Progress",
                    ToolsUsed = "Autopsy / FTK Imager",
                    Findings = "Initial acquisition completed; review in progress."
                }
            );
            ...
            db.Notifications.AddRange(
                new Notification
                {
                    Recipient = "Wasi Ahbab",
                    Title = "Evidence awaiting analysis",
                    Message = "EV-002 is ready to be assigned for forensic examination.",
                    Severity = "Info"
                },
                ...
                new Notification
                {
                    Recipient = "Wasi Ahbab",
                    Title = "Priority case",
                    Message = "CF-2026-001 is a high-priority active investigation.",
                    Severity = "Warning"
                }
            );
            ...
            db.AuditLogs.Add(
                new AuditLog
                {
                    ...
                    Entity = "System",
                    Details = "TraceLock evidence workspace initialized with demo records."
                }
            );
    ...
    private static async Task UpgradeExistingSqliteAsync(EvidenceDbContext db)
    {
        ...
        var additions = new Dictionary<string, Dictionary<string, string>>
        {
            ...
            ["Cases"] = new()
            {
                ["CaseType"] = "TEXT NOT NULL DEFAULT 'Cybercrime'",
            ...
            {
                ["Description"] = "TEXT NOT NULL DEFAULT ''",
                ["SourceDevice"] = "TEXT NOT NULL DEFAULT ''",
                ["AssociatedPerson"] = "TEXT NOT NULL DEFAULT ''",
                ["CollectionLocation"] = "TEXT NOT NULL DEFAULT ''",
                ["Collector"] = "TEXT NOT NULL DEFAULT ''",
                ["CurrentCustodian"] = "TEXT NOT NULL DEFAULT ''",
                ["HashSha256"] = "TEXT NOT NULL DEFAULT ''",
                ["HashMd5"] = "TEXT NOT NULL DEFAULT ''",
                ["HashGeneratedAt"] = "TEXT NULL",
                ["IntegrityStatus"] = "TEXT NOT NULL DEFAULT 'Not verified'",
                ["FileReference"] = "TEXT NOT NULL DEFAULT ''",
            ...
            ["CustodyRecords"] = new()
            {
                ...
                ["Condition"] = "TEXT NOT NULL DEFAULT 'Sealed / intact'",
                ["Authorization"] = "TEXT NOT NULL DEFAULT ''",
                ["Notes"] = "TEXT NOT NULL DEFAULT ''",
                ["RecordedAt"] = "TEXT NOT NULL DEFAULT '2000-01-01 00:00:00'"
            },
        ...
        foreach (var pair in additions)
        {
            ...
            foreach (var column in pair.Value)
            {
                ...
                alter.CommandText =
                    $"ALTER TABLE [{pair.Key}] ADD COLUMN [{column.Key}] {column.Value}";
